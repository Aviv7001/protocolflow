...
class MasterMixManagerScreen extends StatefulWidget {
  ...
  const MasterMixManagerScreen({super.key, required this.wizard, required this.onUpdate});
...
class _MasterMixManagerScreenState extends State<MasterMixManagerScreen> {
  ...
  void _removeReagent(int index) {
    setState(() {
      final newReagents = List<MasterMixReagentItem>.from(_wizard.reagents)..removeAt(index);
  ...
  Widget build(BuildContext context) {
    final res = _calculator.calculateMasterMix(MasterMixInput(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ..._wizard.reagents.asMap().entries.map((e) => _buildReagentEditor(e.key, e.value, res)),
            if (_wizard.reagents.isNotEmpty)
              Padding(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            _resItemEditable(
              'V2 (Total Mix)',
              res.optimizedFinalVolumeUl,
              (val) {
                final roundedV2 = (val * 10).round() / 10.0;
                setState(() => _wizard = _wizard.copyWith(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _DelayedTextField(
              decoration: const InputDecoration(labelText: 'Mix Name / Table Title', border: OutlineInputBorder()),
              initialValue: _wizard.mixName,
              style: const TextStyle(fontSize: _uniformFontSize),
              onCommit: (v) => setState(() => _wizard = _wizard.copyWith(mixName: v)),
            ),
            const SizedBox(height: 12),
            Row(
              children: [
                Expanded(
                  ...
                  child: _DelayedTextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Final Volume', border: OutlineInputBorder()),
                    style: const TextStyle(fontSize: _uniformFontSize),
                    onCommit: (v) => setState(() => _wizard = _wizard.copyWith(finalVolume: double.tryParse(v) ?? 0)),
                  ),
                ),
                ...
                Expanded(
                  child: DropdownButtonFormField<VolumeUnit>(
                    ...
                    onChanged: (v) => setState(() => _wizard = _wizard.copyWith(finalVolumeUnit: v!)),
                  ),
                ),
              ],
            ),
            ...
            _DelayedTextField(
              ...
              decoration: const InputDecoration(labelText: 'Base Solvent Name', border: OutlineInputBorder()),
              style: const TextStyle(fontSize: _uniformFontSize),
              onCommit: (v) => setState(() => _wizard = _wizard.copyWith(baseSolventName: v)),
            ),
          ],
        ),
      ),
  ...
  Widget _buildReagentEditor(int index, MasterMixReagentItem item, MasterMixResult res) {
      ...
      child: Padding(
        ...
        child: Column(
          children: [
              ...
              children: [
                Expanded(
                  child: _DelayedTextField(
                    ...
                    decoration: const InputDecoration(labelText: 'Reagent Name', border: UnderlineInputBorder()),
            ...
            const SizedBox(height: 16),
            _buildConcRow('C1 (Stock Concentration)', item.stockConc, item.stockUnit, 
              (v) => _updateReagent(index, item.copyWith(stockConc: v)), 
              (u) => _updateReagent(index, item.copyWith(stockUnit: u))),
            const SizedBox(height: 12),
            _buildConcRow('C2 (Final Concentration)', item.finalConc, item.finalUnit, 
  ...
  Widget _buildReagentResultPreview(int index, MasterMixReagentItem item, MasterMixReagentResult rRes, MasterMixResult globalRes) {
        ...
        if (isMass)
          _resItemEditableMass(
            ...
            (val) {
              ...
              final double targetConc = isStockMW ? item.finalConc : item.stockConc;
              final ConcentrationUnit targetUnit = isStockMW ? item.finalUnit : item.stockUnit;
              
              double newV2L = 0;
              if (targetUnit.index < 5) { // Molar
                final double m = targetConc * [1, 1e-3, 1e-6, 1e-9, 1e-12][targetUnit.index];
                newV2L = val / (mw * m);
          ...
          _resItemEditable(
            ...
            (val) {
              ...
              if (stockFamily == finalFamily) {
                final stockBase = _convertToBaseConc(item.stockConc, item.stockUnit);
                final finalBase = _convertToBaseConc(item.finalConc, item.finalUnit);
