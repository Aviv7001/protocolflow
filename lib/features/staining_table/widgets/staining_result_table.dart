    ...
    return Card(
      ...
      child: SingleChildScrollView(
        ...
        child: DataTable(
          ...
          rows: [
            ...result.rows.map((row) => DataRow(
                  cells: [
                    DataCell(Text(row.rowName, style: const TextStyle(fontWeight: FontWeight.w500))),
                    DataCell(Text(row.totalStainsText, style: TextStyle(fontSize: 11, color: Colors.grey.shade700))),
                        ...
                        Center(
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: isPositive ? Colors.green.shade500 : Colors.red.shade400,
            ...
            // Metadata Rows
            ...result.metadataRows.map((row) => DataRow(
                  color: WidgetStateProperty.all(Colors.grey.shade50),
                  cells: [
                    DataCell(Text(row.rowName,
                        style: const TextStyle(fontWeight: FontWeight.bold, fontStyle: FontStyle.italic, fontSize: 12))),
                    const DataCell(Text('')),
                    ...result.stainColumns.map((col) {
                        ...
                        Center(
                            child: Text(val,
                                style: const TextStyle(
                                    fontSize: 11, fontWeight: FontWeight.bold, color: Colors.blueGrey))),
                      );
                    }),
                  ],
                )),
